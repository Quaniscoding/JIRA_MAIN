...
export default function KanbanBoard({ listProjectDetails, onTaskClick, setLoading, setProgress, projectId, task }) {
    ...
    useEffect(() => {
        if (listProjectDetails[0]?.listTask) {
            setBoardData(listProjectDetails[0]?.listTask);
        }
    }, [listProjectDetails[0]]);
    ...
    const handleDrop = async (e, dropStatusIndex) => {
        ...
        const draggedItem = newBoardData[statusIndex].listTaskDetail[taskIndex];

        newBoardData[statusIndex].listTaskDetail.splice(taskIndex, 1);

        newBoardData[dropStatusIndex].listTaskDetail.push(draggedItem);
        let isMounted = true;
        setLoading(true);
        setProgress(10);
        try {
            ...
            const result = await dispatch(callEditTask({ projectId: projectId, taskId: draggedItem.id, statusId: newBoardData[dropStatusIndex].statusId }));
        ...
        >
            {boardData?.map((item, statusIndex) => (
                ...
                >
                    ...
                    {item?.listTaskDetail.map((task, taskIndex) => (
                        console.log(task),
                        ...
                        >
                            ...
                            <Box display="flex" mt={1} sx={{ gap: 1 }}>
                                {task.listUserAssign.map((assignee, index) => (
                                    <Tooltip key={index} title={assignee.username}>
                                        <Avatar
                                            src={assignee.avatar}
                            ...
                            >
                                {task.listComment.length} comments
                            </Typography>
                        </Box>
                    ))}
                </Paper>
