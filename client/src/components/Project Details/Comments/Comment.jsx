...
import { callInsertComment } from '../../../redux/reducers/comment/insertComment.js';
...
export default function Comment({ task, setListProjectDetails }) {
    ...
    const listComment = useSelector((state) => state.getTaskDetail.listTaskDetail);
    ...
    useEffect(() => {
        if (task?.id) {
            dispatch(callGetTaskDetail(task.id));
        }
    }, [dispatch, task?.id]);
    ...
    const handleComment = async (e) => {
        ...
        try {
            const result = await dispatch(
                callInsertComment({ contentComment: contentComment.trim(), taskId: task.id })
            );
            if (result.isInsert) {
                await dispatch(callGetTaskDetail(task.id));
                setListProjectDetails(await dispatch(callGetListProjectDetail(task.projectId)));
            }
            setContentComment("");
        } catch (error) {
    ...
    const handleSaveEdit = async (commentId) => {
        ...
        try {
            const result = await dispatch(callUpdateComment(commentId, { contentComment: editingContent.trim(), taskId: task.id }));
            if (result.isUpdated) {
                setSnackbar({
                    open: true,
    ...
    const handleDeleteComment = async () => {
        try {
            const result = await dispatch(callDeleteComment(selectedCommentId, task.id));
            if (result.isDeleted) {
                setSnackbar({
                    open: true,
                        ...
                        >
                            ...
                            <Box sx={{ ml: 2 }}>
                                ...
                                {isEditingCommentId === comment.id ? (
                                    ...
                                    <Typography variant="body2">{comment.contentComment}</Typography>
                                )}
                            </Box>
                            <Box sx={{ mt: 1, display: 'flex', gap: 1 }}>
                                {isEditingCommentId === comment.id ? (
                                    ...
                                    <>
                                        <IconButton color="primary" onClick={() => handleEditComment(comment.id, comment.contentComment)}>
                                            <EditIcon />
                                        </IconButton>
