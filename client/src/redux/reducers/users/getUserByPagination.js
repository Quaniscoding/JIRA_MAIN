...
export const callGetUserByPagination = (page, limit, keyWord, sortBy, sort) => {
    return async (dispatch) => {
        try {
            if (!keyWord) {
                const response = await http.get(`/user/pagination?page=${page}&limit=${limit}&sortBy=${sortBy}&sort=${sort}`);
                dispatch(getListUser(response.data.content));
                return response.data.content
            }
            const response = await http.get(`/user/pagination?page=${page}&limit=${limit}&keyWord=${keyWord}&sortBy=${sortBy}&sort=${sort}`);
            dispatch(getListUser(response.data.content));
            return response.data.content
